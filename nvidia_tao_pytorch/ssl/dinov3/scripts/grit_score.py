...
def main(cfg: ExperimentConfig) -> None:
    ...
    results_dir = cfg.grit_score.results_dir
    if not results_dir and cfg.results_dir:
        results_dir = Path(cfg.results_dir) / "grit_score"
    if not results_dir:
        raise ValueError(
            "You need to set at least one of results_dir or grit_score.results_dir"
        )
    resolved_results = Path(results_dir).expanduser().resolve()
    if (resolved_results / "_SUCCESS").exists():
        raise RuntimeError(
            f"Refusing to overwrite committed output directory: {resolved_results}"
        )
    cfg.grit_score.results_dir = str(resolved_results)
    cfg.results_dir = str(resolved_results)
    _run_experiment(cfg)
