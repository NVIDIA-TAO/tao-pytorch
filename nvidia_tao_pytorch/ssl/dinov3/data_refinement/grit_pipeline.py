    ...
    *,
    ...
    try:
        for query_start in range(0, len(query), block_rows):
            ...
            query_block = F.normalize(query_block, dim=-1)
            ...
            for reference_start in range(0, len(reference), block_rows):
                ...
                reference_block = F.normalize(reference_block, dim=-1)
...
def _validate_runtime_device(value: object, name: str) -> torch.device:
    """Resolve a supported runtime device before loading a checkpoint."""
    try:
        device = torch.device(value)
    except (RuntimeError, TypeError, ValueError) as exc:
        raise ValueError(f"GRIT {name} is invalid: {value!r}") from exc
    if device.type not in {"cpu", "cuda"}:
        raise ValueError(f"GRIT {name} must use cpu or cuda, got {device}")
    if device.type == "cuda":
        device_count = int(torch.cuda.device_count())
        if not torch.cuda.is_available() or device_count == 0:
            raise ValueError(f"GRIT {name} requests CUDA, but CUDA is unavailable")
        requested_index = device.index
        if isinstance(value, str) and value.startswith("cuda:"):
            try:
                requested_index = int(value.partition(":")[2])
            except ValueError as exc:
                raise ValueError(f"GRIT {name} is invalid: {value!r}") from exc
        if (
            requested_index is not None and
            (requested_index < 0 or requested_index >= device_count)
        ):
            raise ValueError(
                f"GRIT {name} requests unavailable CUDA device "
                f"{requested_index}; visible device count is {device_count}"
            )
    return device


def _validate_grit_cohorts(
    manifest: pd.DataFrame,
    settling_k: int,
    view_ks: tuple[int, ...] | list[int],
) -> tuple[int, tuple[int, ...]]:
    """Reject impossible GRIT neighborhoods before checkpoint inference."""
    if isinstance(settling_k, bool) or not isinstance(settling_k, (int, np.integer)):
        raise ValueError("GRIT settling_k must be a positive integer")
    normalized_settling_k = int(settling_k)
    if normalized_settling_k <= 0:
        raise ValueError("GRIT settling_k must be a positive integer")
    if isinstance(view_ks, (str, bytes)):
        raise ValueError("GRIT view_ks must be a non-empty sequence of positive integers")
    try:
        normalized_view_ks = tuple(view_ks)
    except TypeError as exc:
        raise ValueError(
            "GRIT view_ks must be a non-empty sequence of positive integers"
        ) from exc
    if not normalized_view_ks or any(
        isinstance(value, bool) or
        not isinstance(value, (int, np.integer)) or
        int(value) <= 0
        for value in normalized_view_ks
    ):
        raise ValueError(
            "GRIT view_ks must be a non-empty sequence of positive integers"
        )
    normalized_view_ks = tuple(map(int, normalized_view_ks))
    roles = manifest["role"].astype(str)
    queries = manifest.loc[roles == "query"]
    if queries.empty:
        raise ValueError("GRIT manifest has no query rows")
    max_view_k = max(normalized_view_ks)
    for task in sorted(set(queries["task"].astype(str))):
        task_rows = manifest[manifest["task"].astype(str) == task]
        task_roles = task_rows["role"].astype(str)
        query_count = int((task_roles == "query").sum())
        reference_count = int((task_roles == "reference").sum())
        if reference_count <= normalized_settling_k:
            raise ValueError(
                f"GRIT task {task!r} requires more than "
                f"{normalized_settling_k} references"
            )
        if query_count <= max_view_k:
            raise ValueError(
                f"GRIT task {task!r} requires more than "
                f"{max_view_k} query samples"
            )
    return normalized_settling_k, normalized_view_ks
...
def derive_grit_observations(
    ...
    *,
    ...
    settling_k, view_ks = _validate_grit_cohorts(
        manifest, settling_k, view_ks
    )
...
class _ManifestImages(Dataset):
    def __init__(
        ...
        *,
        ...
        self.reader = LocalImageReader(archive_cache_size=archive_cache_size)
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor | int]:
        image = self.reader.image(
            self.storage_types[index], self.paths[index], self.members[index]
        ).resize(
            (self.input_size, self.input_size), Image.Resampling.BICUBIC
        )
        ...
        if self.views:
            crop_size = int(round(self.input_size * 0.9))
            for corner, name in ((0, "view0"), (self.input_size - crop_size, "view1")):
                crop = image.crop(
                    (corner, corner, corner + crop_size, corner + crop_size)
                ).resize((self.input_size, self.input_size), Image.Resampling.BICUBIC)
...
def _build_backbone(base_spec: str | Path, checkpoint: str | Path) -> torch.nn.Module:
    ...
    backbone_type = str(spec.model.backbone.teacher_type)
...
def extract_grit_scores(config: dict) -> pd.DataFrame:
    ...
    required = {"sample_id", "path", "storage_type", "task", "role"}
    missing = required.difference(available)
    if missing:
        raise ValueError(f"GRIT manifest is missing columns: {sorted(missing)}")
    for name in sorted(required):
        if (
            manifest[name].isnull().any() or
            manifest[name].astype(str).str.strip().eq("").any()
        ):
            raise ValueError(
                f"GRIT {name} values must be non-null and non-empty"
            )
    if manifest["sample_id"].duplicated().any():
        raise ValueError("GRIT sample_id values must be globally unique")
    roles = set(manifest["role"].astype(str))
    unsupported_roles = roles.difference({"query", "reference"})
    if unsupported_roles:
        raise ValueError(f"Unsupported GRIT roles: {sorted(unsupported_roles)}")
    ...
    if archive_rows.any() and (
        "member" not in manifest or
        manifest.loc[archive_rows, "member"].isnull().any() or
        manifest.loc[archive_rows, "member"].astype(str).str.strip().eq("").any()
    ):
        raise ValueError(
            "Archive-backed GRIT rows require a non-null, non-empty member"
        )
    device = _validate_runtime_device(config.get("device", "cuda"), "device")
    neighbor_device = _validate_runtime_device(
        config.get("neighbor_device", str(device)), "neighbor_device"
    )
    batch_size = int(config.get("batch_size", 12))
    if batch_size <= 0:
        raise ValueError("GRIT batch_size must be positive")
    workers = int(config.get("workers", 8))
    if workers < 0:
        raise ValueError("GRIT workers must be nonnegative")
    ...
    if input_size != INPUT_SIZE:
        raise ValueError(f"GRIT input_size must be {INPUT_SIZE}, got {input_size}")
    neighbor_block_rows = int(
        config.get("neighbor_block_rows", NEIGHBOR_BLOCK_ROWS)
    )
    if neighbor_block_rows <= 0:
        raise ValueError("GRIT neighbor_block_rows must be positive")
    scratch_headroom = float(config.get("scratch_headroom_fraction", 0.10))
    if not math.isfinite(scratch_headroom) or scratch_headroom < 0:
        raise ValueError(
            "GRIT scratch_headroom_fraction must be finite and nonnegative"
        )
    settling_k, view_ks = _validate_grit_cohorts(
        manifest,
        config.get("settling_k", 50),
        config.get("view_ks", (8, 16, 32)),
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        result = derive_grit_observations(
            ...
            neighbor_block_rows=neighbor_block_rows,
            neighbor_backend=str(config.get("neighbor_backend", "auto")),
        )
