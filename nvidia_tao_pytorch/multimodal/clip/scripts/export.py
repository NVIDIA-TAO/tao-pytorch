...
def run_export(experiment_config: ExperimentConfig) -> None:
    ...
    # Merge LoRA weights into base model if present (PEFT mode).
    # Only runs if the loaded checkpoint contains LoRALinear modules;
    # for standard SFT checkpoints this is a no-op.
    has_lora = any(
        isinstance(m, LoRALinear) for m in pl_model.modules()
    )
    if has_lora:
        merged = merge_lora(pl_model)
        logging.info(
            f"Merged {merged} LoRA modules into base weights for export."
        )
