...
from nvidia_tao_pytorch.multimodal.video_clip.model.builders import (
    build_radio_model,
    build_siglip2_model,
...
class VideoCLIPModelPreProcess():
    ...
    def __init__(self, model, tokenizer, preprocess_train, preprocess_val):
        """Initialize VideoCLIPModelPreProcess."""
        self.model = model
...
def build_model(experiment_config,
    ...
    Returns:
        VideoCLIPModelPreProcess: A wrapper object containing the model,
    ...
    if model_type in internvideo2clip_model_configs:
    ...
    elif model_type in siglip2_model_configs:
        model, preprocess_train, preprocess_val, tokenizer = (
            build_siglip2_model(
                model_version=model_type,
                aug_cfg=dict(aug_cfg) if aug_cfg is not None else None,
    ...
    elif model_type in openclip_model_configs:
        model, preprocess_train, preprocess_val, tokenizer = (
            build_openclip_model(
                model_version=model_type,
                aug_cfg=dict(aug_cfg) if aug_cfg is not None else None,
        ...
        model_config = map_clip_model_cfg.get(model_type)

        # Handle customized clip config
        if model_config is not None:
            open_clip.factory._MODEL_CONFIGS[model_type] = model_config
    ...
    return VideoCLIPModelPreProcess(
