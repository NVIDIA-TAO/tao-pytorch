...
"""InternVideo2 vision tower: a video ViT pooled into the CLIP embedding space."""
import logging
import math
import torch
...
class CrossAttention(nn.Module):
    """Multi-head cross-attention between query tokens and key/value tokens."""
    ...
    def forward(self, x, k=None, v=None):
        """Attend the query tokens to the key/value tokens."""
        B, N, _ = x.shape
        N_k = k.shape[1]
        N_v = v.shape[1]
...
class AttentiveBlock(nn.Module):
    """Normalization plus cross-attention block used by the attentive pooler."""
    ...
    def forward(self, x_q, x_kv, pos_q, pos_k, bool_masked_pos, rel_pos_bias=None):
        """Normalize the query and key/value streams, then cross-attend."""
...
class AttentionPoolingBlock(AttentiveBlock):
    """Attentive pooler reducing a token sequence to a single embedding."""

    def forward(self, x):
        """Pool the token sequence using its mean as the attention query."""
...
class RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        """Normalize the hidden states by their root mean square in fp32."""
...
class LayerScale(nn.Module):
    """Per-channel learnable scaling applied to a residual branch."""
    ...
    def forward(self, x):
        """Scale the input by the learned gamma, optionally forcing fp32."""
        if self.force_fp32:
            output_type = x.dtype
...
class Attention(nn.Module):
    """Multi-head self-attention with optional flash attention and QK normalization."""
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., use_flash_attn=False,
        ...
        # use_flash_attn/causal/use_fused_rmsnorm kwargs are accepted for
        # config/back-compat but no longer branch: F.scaled_dot_product_attention
        # dispatches to the flash / mem-efficient kernel itself.
        self.qk_normalization = qk_normalization
        self.q_norm = norm_layer(dim) if qk_normalization else nn.Identity()
        self.k_norm = norm_layer(dim) if qk_normalization else nn.Identity()

    def forward(self, x):
        """Run multi-head self-attention over the token sequence."""
        B, N, C = x.shape
...
class Block(nn.Module):
    """Transformer block with LayerScale on the attention and MLP residual branches."""
    ...
    def forward(self, x, residual=None):
        """Run the attention and MLP residual branches, optionally checkpointed."""
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        """Embed the input video into spatio-temporal patch tokens."""
        x = self.proj(x)
...
class InternVideo2(nn.Module):
    """InternVideo2 video vision transformer with an attentive-pooling CLIP head."""
    ...
    def init_pos_embed(self):
        """Initialize the position embeddings from a fixed sin-cos table."""
    ...
    def fix_init_weight(self):
        """Rescale each block's projection weights by its layer depth."""
    ...
    @property
    def dtype(self):
        """Return the dtype of the patch-embedding weights."""
    ...
    def no_weight_decay(self):
        """Return the parameter names that must be excluded from weight decay."""
        return {
            'pos_embed',
            'pos_embed_spatial',
            'pos_embed_temporal',
    ...
    def forward(self, x, use_image=False):
        """Encode a video clip into patch tokens and pool them to a CLIP embedding."""
