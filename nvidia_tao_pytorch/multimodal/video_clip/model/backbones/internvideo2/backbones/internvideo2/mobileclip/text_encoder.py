...
#
"""MobileCLIP text tower used as the InternVideo2-CLIP text encoder."""
import math
...
class TextTransformer(nn.Module):
    """Transformer text encoder projecting tokens into the CLIP embedding space."""
    ...
    def __init__(self, cfg: dict, projection_dim: int, *args, **kwargs) -> None:
        ...
        if not isinstance(ffn_multipliers, Sequence):
            ...
            isinstance(ffn_multipliers, Sequence) and
    ...
    def encode_text(  # pylint: disable=keyword-arg-before-vararg
        self,
        text_tokens: Tensor,
    ...
    def forward(  # pylint: disable=keyword-arg-before-vararg
        ...
        **kwargs
        ...
        # [B, CL] --> [B, d]
        """Encode the token sequence and project the EOT token into the CLIP space."""
