...
"""InternVideo2-CLIP dual-tower model: InternVideo2 vision + MobileCLIP text."""
import logging
import os
import json
...
class InternVideo2_CLIP_small(nn.Module):
    """InternVideo2-CLIP model pairing an InternVideo2 vision tower with a MobileCLIP text tower."""
    ...
    def __init__(self, config, tokenizer=None, is_pretrain=True):
        ...
        text_encoder_cfg = json.load(
            open(os.path.join(
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "backbones", "internvideo2", "mobileclip", "configs") + "/" +
        ...
        if any(checkpoint_paths):
            self.load_checkpoint(*checkpoint_paths)
        else:
            logger.info(
                "No InternVideo2-CLIP component checkpoints configured; "
                "keeping the initialized architecture."
            )
    ...
    def load_checkpoint(self, vision_ckpt_path=None, text_ckpt_path=None, extra_ckpt_path=None):
        """Load the vision, text and extra checkpoints into the model."""
