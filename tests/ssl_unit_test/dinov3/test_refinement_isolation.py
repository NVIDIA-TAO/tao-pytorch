...
def test_real_lightning_publishes_authoritative_terminal_teacher(tmp_path):
    """The checkpoint producer, not a downstream step estimate, owns the extent."""
    class TinyModule(pl.LightningModule):
        def __init__(self):
            super().__init__()
            self.student = torch.nn.ModuleDict({"backbone": torch.nn.Linear(1, 1)})
        ...
        def training_step(self, batch, batch_idx):
            del batch_idx
            return self.student["backbone"](batch[0]).square().mean()

        def configure_optimizers(self):
            return torch.optim.SGD(self.student.parameters(), lr=0.1)

    callback = DinoV3ModelCheckpoint(
        dirpath=tmp_path, every_n_epochs=10, save_top_k=-1, save_final_epoch=True,
        publish_terminal_manifest=True,
    )
    trainer = pl.Trainer(
        accelerator="cpu", devices=1, default_root_dir=tmp_path, max_epochs=1,
        callbacks=[callback], logger=False, enable_model_summary=False,
        enable_progress_bar=False, num_sanity_val_steps=0,
    )
    trainer.fit(TinyModule(), train_dataloaders=DataLoader(TensorDataset(torch.ones(3, 1))))
    marker = json.loads((tmp_path / "terminal_teacher.json").read_text())
    checkpoint = tmp_path / marker["filename"]
    assert marker["model"] == "teacher"
    assert marker["epoch"] == 0
    assert marker["global_step"] == trainer.global_step == 3
    assert marker["bytes"] == checkpoint.stat().st_size
    assert marker["sha256"] == "sha256:" + hashlib.sha256(checkpoint.read_bytes()).hexdigest()
    assert set(torch.load(checkpoint, weights_only=True)) == {"weight", "bias"}


def test_terminal_marker_rejects_a_stale_teacher_extent(tmp_path):
    """A previously saved periodic teacher cannot attest a later training step."""
    callback = DinoV3ModelCheckpoint(dirpath=tmp_path, publish_terminal_manifest=True)
    callback._teacher_export = {"filename": "teacher.pth", "epoch": 0, "global_step": 2}
    trainer = SimpleNamespace(is_global_zero=True, global_step=3)
    with pytest.raises(ValueError, match="terminal teacher export"):
        callback.on_train_end(trainer, None)
    assert not (tmp_path / "terminal_teacher.json").exists()
