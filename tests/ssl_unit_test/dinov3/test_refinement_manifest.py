...
@pytest.mark.parametrize("rows,ranks,batch_size", [(5, 1, 2), (11, 3, 3), (2, 4, 2)])
def test_manifest_sampler_pads_full_batches_without_losing_rows(rows, ranks, batch_size):
    """KoLeo must never see a singleton tail, including underfilled ranks."""
    ...
    class Dataset:
        ...
        @staticmethod
        def sampling_group(index):
            return f"tar:{index // 3}"

    partitions = []
    for rank in range(ranks):
        sampler = ShardAwareDistributedSampler(
            Dataset(), num_replicas=ranks, rank=rank, batch_size=batch_size,
        )
        batches = list(BatchSampler(sampler, batch_size, drop_last=True))
        assert all(len(batch) == batch_size for batch in batches)
        partition = [row for batch in batches for row in batch]
        assert len(partition) == len(sampler)
        assert list(sampler) == partition
...
def test_relative_manifest_paths_do_not_depend_on_working_directory(tmp_path, monkeypatch):
    """An unspecified images_dir means the manifest directory, not cwd."""
    image = tmp_path / "image.png"
    Image.new("RGB", (4, 4), "green").save(image)
    manifest = _manifest(tmp_path / "train.parquet", [{
        "sample_id": "image", "storage_type": "file", "path": "image.png",
    }])
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)
    dataset = DinoV3Dataset(root="", manifest_path=manifest, transform=_transform)
